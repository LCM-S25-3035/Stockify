...
def load_agent_data(agent_name):
    """
    Load tickers, price data, indicators, scalers, and a PPO model for a given agent.
    """
...
class AgentEnv(gym.Env):
    """
    Environment for asset-specific agent using micro/macro indicators and market regimes.
    """
...
class MetaPortfolioEnv(gym.Env):
    """
    Meta-level Gym environment for combining agent-level portfolio allocations.
    """
...
if __name__ == "__main__":
    ...
    for name, env in zip(["Stock", "Crypto", "ETF"], [env_stock, env_crypto, env_etf]):
        env.reset()
        print(f"{name} Prices shape:", env.prices.shape)
        print(f"{name} Micro indicators shape:", env.micro_indicators.shape)
        print(f"{name} Expected obs shape:", env.observation_space.shape)
        print(f"{name} Sample obs shape:", env._next_observation().shape)
